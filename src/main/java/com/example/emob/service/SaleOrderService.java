...
public class SaleOrderService implements ISaleOrder {
    ...
    public APIResponse<SaleOrderResponse> createSaleOrderFromQuotation(
        ...
        Set<UUID> quotationItemIds = itemRequests.stream()
                .map(SaleOrderItemRequest::getItemsId)
                .collect(Collectors.toSet());

        // ✅ Tạo danh sách SaleOrderItem từ QuotationItem
        Set<SaleOrderItem> saleOrderItems = quotation.getQuotationItems().stream()
                .filter(item -> quotationItemIds.contains(item.getId()))
                .map(item -> {
                    // Tìm request tương quotationItemIds
                    SaleOrderItemRequest req = itemRequests.stream()
                            .filter(r -> r.getItemsId().equals(item.getId()))
                            .findFirst()
                            .orElseThrow(() -> new GlobalException(ErrorCode.INVALID_CODE, "Invalid item request"));
                    ...
                    BigDecimal discountedPrice = PromotionHelper.calculateDiscountedPrice(unitPrice, promotion, quotation.getCustomer());
        ...
        BigDecimal totalAmount = saleOrderItems.stream()
        ...
        // ✅ Tạo response
        SaleOrderResponse response = saleOrderMapper.toSaleOrderResponse(saleOrder);
    ...
    public APIResponse<SaleOrderResponse> deleteSaleOrderById(UUID saleOrderId) {
        ...
        return APIResponse.success(saleOrderMapper.toSaleOrderResponse(saleOrderRepository.save(saleOrder)), "Sale order canceled successfully");
    }
    ...
    @Override
    public APIResponse<PageResponse<SaleOrderResponse>> getAllSaleOrders(Pageable pageable) {
        Dealer dealer = AccountUtil.getCurrentUser().getDealer();
        Page<SaleOrder> page = saleOrderRepository.findAllByDealer(dealer,pageable);
