...
public class TestDriveService implements ITestDrive {
  ...
  public APIResponse<TestDriveResponse> createSchedule(TestDriveRequest request) {
    ...
    VehicleUnit vehicle = vehicleUnitRepository
            .findById(request.getTestDriveVehicleId())
            .orElseThrow(() -> new GlobalException(ErrorCode.NOT_FOUND, "Electric vehicle not found"));

    if (!VehicleStatus.TEST_DRIVE.equals(vehicle.getStatus())) {
      throw new GlobalException(ErrorCode.DATA_INVALID, "This vehicle is not available for test drive");
    }
  ...
  public APIResponse<TestDriveResponse> updateSchedule(TestDriveRequest request, UUID id) {
    ...
    //tìm xe
    TestDrive testDrive = testDriveRepository
            .findById(id)
            .orElseThrow(() -> new GlobalException(ErrorCode.NOT_FOUND, "Test drive schedule not found"));
    //cập nhật
    LocalDateTime scheduledAt = request.getScheduledAt();
    LocalTime time = scheduledAt.toLocalTime();
  ...
  public APIResponse<TestDriveResponse> viewSchedule(UUID id) {
    ...
    return APIResponse.success(testDriveMapper.toTestDriveResponse(testDrive), "View schedule successfully");
  }
  ...
  public APIResponse<TestDriveResponse> cancelSchedule(UUID id) {
    ...
    try {
      ...
      testDriveRepository.save(testDrive);
      return APIResponse.success(testDriveMapper.toTestDriveResponse(testDrive), "Cancel schedule successfully");
    } catch (DataIntegrityViolationException ex) {
      throw new GlobalException(ErrorCode.DATA_INVALID);
    } catch (DataAccessException ex) {
      throw new GlobalException(ErrorCode.DB_ERROR);
  ...
  public APIResponse<PageResponse<TestDriveResponse>> viewAllSchedules(
    ...
    Page<TestDrive> testDrives = testDriveRepository.searchAndFilter(AccountUtil.getCurrentUser().getDealer(),keyword, status, pageable);
  ...
  public APIResponse<PageResponse<TestDriveResponse>> viewAllSchedulesByStaff(
    ...
    Page<TestDrive> testDrives = testDriveRepository.searchAndFilterByStaff(AccountUtil.getCurrentUser().getDealer(),AccountUtil.getCurrentUser(),keyword, status, pageable);
    PageResponse<TestDriveResponse> response =
            pageMapper.toPageResponse(testDrives, testDriveMapper::toTestDriveResponse);
    return APIResponse.success(response, "View all schedules successfully");
  }
  ...
  @PreAuthorize("hasRole('DEALER_STAFF')")
  public APIResponse<List<VehicleUnitResponse>> getFreeVehiclesByDate(LocalDateTime scheduledAt, int duration, String model) {
    ...
    List<VehicleUnit> vehicleUnits = vehicleUnitRepository.findAvailableVehiclesByTimeRangeAndModel(startAt, endAt,model);
  ...
  private boolean checkFreeTime(LocalDateTime scheduledAt, Account staff, int duration, VehicleUnit vehicleUnit) {
