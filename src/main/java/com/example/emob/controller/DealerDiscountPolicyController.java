...
public class DealerDiscountPolicyController {
    ...
    public ResponseEntity<APIResponse<List<DealerDiscountPolicyResponse>>> bulkCreate(
        ...
        APIResponse<List<DealerDiscountPolicyResponse>> responses = dealerDiscountPolicyService.bulkCreate(request);
    ...
    @Operation(
            ...
            description = "Set status to INACTIVE for multiple DealerDiscountPolicies based on dealerIds and vehicleModelIds",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "DealerDiscountPolicyBulkDeleteRequest payload",
                    required = true,
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = DealerDiscountPolicyBulkDeleteRequest.class),
                            examples = {
                                    @ExampleObject(
    ...
    public ResponseEntity<APIResponse<List<DealerDiscountPolicyResponse>>> bulkDelete(
        ...
        APIResponse<List<DealerDiscountPolicyResponse>> response = dealerDiscountPolicyService.bulkDelete(request);
    ...
    public ResponseEntity<APIResponse<DealerDiscountPolicyResponse>> delete(
    ...
    public ResponseEntity<APIResponse<DealerDiscountPolicyResponse>> update(
            @PathVariable UUID id,
            @RequestBody @Valid DealerDiscountPolicyRequest request) {
        APIResponse<DealerDiscountPolicyResponse> response = dealerDiscountPolicyService.update(id, request);
        return ResponseEntity.ok(response);
    }

    // =================  Get All =================
    @GetMapping
    @Operation(summary = "Get all Dealer Discount Policies", description = "Retrieve paginated list of Dealer Discount Policies")
    public ResponseEntity<APIResponse<PageResponse<DealerDiscountPolicyResponse>>> getAll(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        Pageable pageable = PageRequest.of(page, size);
        APIResponse<PageResponse<DealerDiscountPolicyResponse>> response = dealerDiscountPolicyService.getAll(pageable);
        return ResponseEntity.ok(response);
    }
